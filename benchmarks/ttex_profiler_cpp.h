    ...
    unsigned int sub_parallel_id)
{
  ...
  if(sub_parallel_id != 0){ // start of work
    ...
    if(parallel_region[parallel_data->value][sub_parallel_id-1].ref > omp_sections_ref && count != -1){ // -1 count means thread enters but not takes any section will go to work end
      ...
      temp_node = parallel_region[parallel_data->value][sub_parallel_id-1].expected_execution[count];
      resetTimer(temp_thread_data->thread_current_timeout[temp_thread_data->thread_current_timeout.size()-1].wcet,temp_thread_data);
    }

    else if (parallel_region[parallel_data->value][sub_parallel_id-1].ref == omp_for_ref){
      temp_node = parallel_region[parallel_data->value][sub_parallel_id-1].expected_execution[0]; // only one vector in other case
      resetTimer(temp_thread_data->thread_current_timeout[temp_thread_data->thread_current_timeout.size()-1].wcet,temp_thread_data);
    }
    ...
    temp_thread_data->timeout_node_id.push_back(0); // maxvuln will pick the last id -- for nested callbackwork/parallelism
  }
    ...
    clock_gettime(CLOCK_MONOTONIC, &temp_thread_data->thread_current_timeout[temp_thread_data->thread_current_timeout.size()-1].et);
    while(temp_thread_data->timeout_node_id[temp_thread_data->timeout_node_id.size()-1] != 0){
      temp_thread_data->timeout_node_id.pop_back();
    }
    ...
    resetTimer(max_timeout,temp_thread_data);
  }
}
...
extern "C" void initializeTimeoutData(){
  ...
  if(logdata){
    ...
    parallel_begin.sub_region_id = parallel_begin_id;
    thread_begin.sub_region_id = thread_begin_id;
    parallel_end.sub_region_id = parallel_end_id;

    parallel_begin.sections_id = parallel_begin_id;
    thread_begin.sections_id = thread_begin_id;
    parallel_end.sections_id = parallel_end_id;

    printf("Printing details\n");

    for(int i = 0 ; i < parallel_size; i++){

      printf("Parallel region id%d\n", i);
      ...
      for(int j = 0 ; j < ast_size[i] ; j++){
        ...
        if(parallel_region[i][j].ref > omp_sections_ref){ //sections
          ...
          for(int k = 0 ; k < parallel_region[i][j].ref ; k++){
            ...
            vector<timeout_node> temp_v;
            timeout_node temp;
            temp.wcet = exec_time;
            temp.sub_region_id = j;
            temp.parallel_region_id = i;
            temp.sections_id = k;
            temp_v.push_back(temp); // that loops last line
            parallel_region[i][j].expected_execution.push_back(temp_v); 
          }
