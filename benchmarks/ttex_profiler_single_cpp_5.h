...
static int lock_work_begin=0;
static int lock_work_end=0;
static int lock_added_callback=0;
...
void timer_handler (int signum)
{
    ...
    printf("***************************** handler called for thread id %d ************\n", temp_thread_data->id);
    //return;
    //pthread_exit(NULL);
}

// void createTimer(thread_info* temp_thread_data){ // Initiate a timer
void createTimer(){
  // temp_thread_data->timer_event.sigev_notify = SIGEV_SIGNAL;
  // temp_thread_data->timer_event.sigev_signo = SIGALRM;
  // memset (&(temp_thread_data->sa), 0, sizeof (temp_thread_data->sa));
  // temp_thread_data->sa.sa_handler = &timer_handler;
  // temp_thread_data->sa.sa_flags = 0;
  // sigaction (SIGALRM, &(temp_thread_data->sa), NULL);

  timer_event.sigev_notify = SIGEV_SIGNAL; // can use a handler with SIGEV_THREAD but that does not generate any signal
  timer_event.sigev_signo = SIGALRM; // only specified with SIGEV_SIGNAL but now also need a sigaction handler with this
  //timer_event.sigev_notify_function = (void*)my_handler;
  struct sigaction sa;
  memset (&sa, 0, sizeof (sa));
  sa.sa_handler = &timer_handler;
  sa.sa_flags = 0; //do not restart system calls if signal is received
  sigaction (SIGALRM, &sa, NULL);
 // timer_create(CLOCK_MONOTONIC, &(temp_thread_data->timer_event), &temp_thread_data->thread_timer_id);
}

void resetTimer(timespec t, thread_info *temp){ 
...
extern "C" int sync_next_added_callback() // until this syncs to num_threads do not proceed (num_threas/ID should be perfectly divisible)
{
  int ret = (int) __sync_fetch_and_add(&lock_added_callback,1);
  //assert(ret<MAX_THREADS && "Maximum number of allowed threads is limited by MAX_THREADS");
  return ret;
}

extern "C" int sync_next_work_end() // until this syncs to num_threads 
{
  int ret = (int) __sync_fetch_and_add(&lock_work_end,1);
  //assert(ret<MAX_THREADS && "Maximum number of allowed threads is limited by MAX_THREADS");
  return ret;
}

extern "C" int sync_next_work_begin() // until this syncs to num_threads
{
  int ret = (int) __sync_fetch_and_add(&lock_work_begin,1);
  //assert(ret<MAX_THREADS && "Maximum number of allowed threads is limited by MAX_THREADS");
  return ret;
}
...
ompt_test (int parallel_region_id, int sub_id)
{
  ...
  thread_info* temp_thread_data = (thread_info*) current_thread->ptr;
    ...
    if(temp_thread_data->id == 0){
        ...
        resetTimer(temp_thread_data->thread_current_timeout[temp_thread_data->thread_current_timeout.size()-1].wcet,temp_thread_data);
        // get parallel id here
    }

    lock_added_callback = sync_next_added_callback();
    while(lock_added_callback % omp_get_num_threads() != 0){
        // do nothing wait 
        printf("ompt test callback\n");
    }
}
    ...
    unsigned int sub_parallel_id)
{
  ...
  printf("----------------------- work callback for---------------------%d\n",temp_thread_data->id);
    ...
    if(temp_thread_data->id == 0){
         if(sub_parallel_id != 0){ // start of work
            ...
            if(parallel_region[parallel_data->value][sub_parallel_id-1].ref > omp_sections_ref && count != -1){ // -1 count means thread enters but not takes any section will go to work end
                ...
                resetTimer(max_timeout, temp_thread_data); // thread will directly enter callback work end or sleep
    ...
    if(sub_parallel_id != 0){
        printf("Before work begin callback is executed\n");
        lock_work_begin = sync_next_work_begin();
        printf("work begin id is: %d\n",lock_work_begin);
        while(lock_work_begin % omp_get_num_threads() != 0){
            // do nothing wait till  value updated
        }
    }

    else {
        printf("Before work begin callback is executed\n");
        lock_work_end = sync_next_work_end();
        while(lock_work_end % omp_get_num_threads() != 0){
            // do nothing wait 
        }
    }
}
  ...
  const void *codeptr_ra)
{
  ...
  if(temp_thread_data->id == 0){  
    ...
    resetTimer(sync_region.wcet,temp_thread_data);
