...
static void __kmp_for_static_init(ident_t *loc, kmp_int32 global_tid,
  ...
  //shared_security_factor = ttex_utility(shared_security_factor, sub_parallel_id);
...
#if OMPT_SUPPORT && OMPT_OPTIONAL
    if (ompt_enabled.ompt_callback_work) {
      // __kmp_printf("Is this callback start 1\n");
      ompt_callbacks.ompt_callback(ompt_callback_work)(
          ompt_work_type, ompt_scope_begin, &(team_info->parallel_data),
          &(task_info->task_data), 0, codeptr, sub_parallel_id);
    }
#endif
...
// #endif
...
#if OMPT_SUPPORT && OMPT_OPTIONAL
    if (ompt_enabled.ompt_callback_work) {
      // __kmp_printf("Is this callback start 2\n");
      ompt_callbacks.ompt_callback(ompt_callback_work)(
          ompt_work_type, ompt_scope_begin, &(team_info->parallel_data),
          &(task_info->task_data), *pstride, codeptr, sub_parallel_id);
    }
#endif
    KMP_STATS_LOOP_END(OMP_loop_static_iterations);
    return;
  ...
  nth = team->t.t_nproc;
  if (nth == 1) {
    // __kmp_printf("------------ Just checking for sections -------------\n");
    if (plastiter != NULL)
      *plastiter = TRUE;
    *pstride =
        (incr > 0) ? (*pupper - *plower + 1) : (-(*plower - *pupper + 1));
...
#if OMPT_SUPPORT && OMPT_OPTIONAL
    if (ompt_enabled.ompt_callback_work) {
      // __kmp_printf("Is this callback start 3\n");
      ompt_callbacks.ompt_callback(ompt_callback_work)(
          ompt_work_type, ompt_scope_begin, &(team_info->parallel_data),
          &(task_info->task_data), *pstride, codeptr, sub_parallel_id);
    }
...
// counter++;
  ...
  switch (schedtype) {
  case kmp_sch_static: {
    // __kmp_printf("schedule type static, trip count is: %d and number of thread is: %d \n", trip_count, nth);
    if (trip_count < nth) {
      // __kmp_printf("!!!!!!!!!!!trip count less than number of threads!!!!!!!\n");
      KMP_DEBUG_ASSERT(
          __kmp_static == kmp_sch_static_greedy ||
          __kmp_static ==
              kmp_sch_static_balanced); // Unknown static scheduling type.
      if (tid < trip_count) {
        // __kmp_printf("tid is %d and trip count is %d\n",tid,trip_count);
     ...
     counter++;
      ...
      if (__kmp_static == kmp_sch_static_balanced) {
        // __kmp_printf("!!!!!!!!!!!kmp static == kmp_sch_static_balanced!!!!!!!\n");
  ...
  case kmp_sch_static_chunked: {
    // __kmp_printf("schedule type static chunked\n");
    ST span;
    UT nchunks;
    if (chunk < 1)
      chunk = 1;
  ...
  case kmp_sch_static_balanced_chunked: {
    // __kmp_printf("schedule type static balanced chunk\n");
    T old_upper = *pupper;
...
//   if(*pupper == *plower)
//     (task_info->task_data).value = *pupper; 
//   else
//     (task_info->task_data).value = -1;
//  }
// }

// task_data is not needed for ttex so utilizing it else could have used another id in task_data.ptr
  //swastik
// if(*pupper && *plower){
//  __kmp_printf("XXXXXXXXXXXXXXXXXXXXXXXXXXxfailsxXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx\n");
//   if(*pupper < *plower)
