...
void __kmpc_fork_call(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...) {
  ...
  // maybe to save thr_state is enough here
  {
    ...
    void (**temp_microtask)(int* temp_gtid, int* temp_tid, ...) = &microtask;
...
void __kmpc_fork_teams(ident_t *loc, kmp_int32 argc, kmpc_micro microtask,
  ...
  __kmp_fork_call(
      ...
      VOLATILE_CAST(launch_t) __kmp_invoke_teams_master, kmp_va_addr_of(ap), 0);
  __kmp_join_call(loc, gtid
#if OMPT_SUPPORT
                  ,
                  fork_context_intel
...
kmp_int32 __kmpc_single(ident_t *loc, kmp_int32 global_tid, kmp_int32 sub_region_id) {
  __kmp_assert_valid_gtid(global_tid);
...
void __kmpc_for_static_fini(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.ompt_callback_work) {
    ...
    ompt_callbacks.ompt_callback(ompt_callback_work)(
        ...
        &(task_info->task_data), 0, OMPT_GET_RETURN_ADDRESS(0),0);
  }
#endif
  if (__kmp_env_consistency_check)
