  ...
  if (ompt_enabled.enabled &&
    ...
    if (ompt_enabled.ompt_callback_parallel_begin) {
      ...
      ompt_callbacks.ompt_callback(ompt_callback_parallel_begin)(
          ...
          ompt_parallel_invoker_program | ompt_parallel_team, codeptr, id);

      // swastik: 2 is just the sample id added by me
    }
  }
#endif // OMPT_SUPPORT
  ...
  { // KMP_TIME_BLOCK
    ...
    if (ompt_enabled.enabled) {
      if (ompt_enabled.ompt_callback_parallel_begin) {
        ...
        kmp_team_t *parent_team_temp; // swastik: lets capture the arguments
        //microtask_t *func_temp = void* (microtask)(int* gtid, int* tid, void* ptr);
        //int* temp_gtid, *temp_tid;
        //void (**func_temp)(temp_gtid, temp_tid, ...) = &microtask;
        // va_list temp_ap;
        // va_start(temp_ap,temp_tid);
        // //__kmp_printf("##checking gtid value: %d##\n", gtid);
        // void* ptr = va_arg(kmp_va_deref(temp_ap), void*);
        // printf("---------- size of ptr %d\n", sizeof(ptr));
        // va_end(temp_ap);
        //int *a = func_temp.gtid;
        //parent_team_temp->t.t_ident = loc;
        // ident_t *loc_t = new ident_t[sizeof(ident_t)];
        // memcpy(loc_t,loc,sizeof(ident_t));
        //parent_team_temp->t.t_ident = loc_t;
        //__kmp_alloc_argv_entries(argc, parent_team_temp, TRUE);
        // parent_team_temp->t.t_argc = argc;
        // void** temp_argv = (void **)parent_team_temp->t.t_argv;
        // for (i = argc - 1; i >= 0; --i)
        // *temp_argv++ = va_arg(kmp_va_deref(ap), void *);
        // int j;
        //for(j = 0; j < i; ++j)
          //printf("%02x\n", ((uint8_t*) *temp_argv)[j]);
        ...
        //int id = 2;
        //cout<<"---------------------------checking id value----------"<<id<<endl;
        //__kmp_printf("------------------ here-----------------------\n");
        //__kmp_printf("------------------------- checking id value------------%d",id);
        ompt_callbacks.ompt_callback(ompt_callback_parallel_begin)(
            parent_task_data, ompt_frame, &ompt_parallel_data, team_size, flags,
            return_address,id); // swastik: 2 is just the addtional added id
      }
     ...
     // __kmp_printf("------------------ here3-----------------------\n");
      ...
      for (i = argc - 1; i >= 0; --i)
        *argv++ = va_arg(kmp_va_deref(ap), void *);
        __kmp_printf("++++++++++++++++++++++++ what is inside argv %s", *argv);
    ...
    //sleep(10); //swastik: threads not spawned till here
...
// don't use lw_taskteam after linking. content was swaped
    ...
    if ((nthreads_icv > 0) || (proc_bind_icv != proc_bind_default)) {
      ...
      /* allocate a new parallel team */
      KF_TRACE(10, ("__kmp_fork_call: before __kmp_allocate_team\n"));
      // sleep(10); //swastik:: function below calls thread begin
      team = __kmp_allocate_team(root, nthreads, nthreads,
#if OMPT_SUPPORT
                                 ompt_parallel_data,
#endif
                                 ...
                                 argc USE_NESTED_HOT_ARG(master_th));
      // sleep(10); stops here after all thread begin
      if (__kmp_barrier_release_pattern[bs_forkjoin_barrier] == bp_dist_bar)
...
// only one notification scheme (either "submit" or "forking/joined", not both)
    ...
    //sleep(10);
    if (ap) {
      __kmp_internal_fork(loc, gtid, team); //swastik: this function starts the work
      //sleep(10);
...
// (they hang on fork barrier until next parallel)
  ...
  __kmp_fork_call(loc, gtid, fork_context_intel, team->t.t_argc,
                  ...
                  VOLATILE_CAST(launch_t) __kmp_invoke_task_func, NULL, 0);
#if INCLUDE_SSC_MARKS
  SSC_MARK_JOINING();
#endif
...
//  __kmp_printf("Checking invoke task execution---------"); // swastik:
  //sleep(10);
...
void __kmp_hidden_helper_threads_initz_routine() {
  ...
  __kmpc_fork_call(nullptr, 0, __kmp_hidden_helper_wrapper_fn, 0);
