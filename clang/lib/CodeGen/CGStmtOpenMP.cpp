...
class TtexClass : public RecursiveASTVisitor<TtexClass> {
  ...
  public:
    ...
    bool TraverseDecl(Decl *D) {
        // your logic here
        RecursiveASTVisitor<TtexClass>::TraverseDecl(D); // Forward to base class
    ...
    bool TraverseStmt(Stmt *x) {
       ...
       //if(x) std::cout<<"------------------- statement class name is: ----------------- "<< x->getStmtClassName()<<std::endl;
        ...
        if(x && strcmp(x->getStmtClassName(),"OMPForDirective") == 0){
          ...
          //   // llvm::Constant* c2 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(*ctxt),0);
          //   // values.push_back(c1);
          //   // values.push_back(c2);
          //   values_temp.push_back(ctr);
          ...
          // if(ctr != 0){
          //   // llvm::Constant* c1 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(*ctxt),ctr);
          //   // llvm::Constant* c2 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(*ctxt),-1);
          //   // values.push_back(c1);
          //   // values.push_back(c2);
          //   values_temp.push_back(ctr);
          //   values_temp.push_back(-1);
          //   ctr = 0;
          // }
          // std::cout<<"((((((((((((( inside parallel directive )))))))))))"<<std::endl;
          //values_temp.push_back(-1);
          //std::cout<<"Parallel Inserted value and new size: "<<values_temp[values_temp.size()-1]<<" "<<values_temp.size()<<std::endl;
          return true;
          //return true;
        }
...
static void emitCommonOMPParallelDirective(
  ...
  // llvm::Constant* c1 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),ctr1); //llvm::Constant::getIntegerValue(llvm::Type::getInt64Ty(context), ctr1);
  // llvm::Constant* c2 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),ctr2); //llvm::Constant::getIntegerValue(ty, ctr2);
  // llvm::Constant* c3 = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),ctr3);// llvm::Constant::getIntegerValue(ty, ctr3);
  // values.push_back(c1);
  // values.push_back(c2);
  // values.push_back(c3);
  // llvm::ArrayType* ar = llvm::ArrayType::get(llvm::Type::getInt64Ty(context), 20);
